...
import com.shtoone.aqxj.activity.StroageDetailActivity;
import com.shtoone.aqxj.adapter.OnItemClickListener;
import com.shtoone.aqxj.adapter.StorageFragmentRecyclerViewAdapter;
...
import com.shtoone.aqxj.bean.StorageFragmentListData;
...
public class StorageFragment extends BaseLazyFragment {
    ...
    private void initData() {
        ...
        if (null != BaseApplication.mDepartmentData && !TextUtils.isEmpty(BaseApplication.mDepartmentData.departmentName)) {
            mDepartmentData = new DepartmentData(mUserInfoData.getDepartId(), mUserInfoData.getDepartName(), ConstantsUtils.STORAGEFRAGMENT);
        }
                    ...
                    if (itemsData.isSuccess() && itemsData.getData().size() > 0) {
                        ...
                        if (null != listData) {
                            if (listData.size() > 0) {
                                ...
                                mScaleInAnimationAdapter = new ScaleInAnimationAdapterFactory().getAdapter(mAdapter = new StorageFragmentRecyclerViewAdapter(_mActivity, listData));
