...
import com.shtoone.aqxj.adapter.QuyangFragmentRVAdapter;
...
import com.shtoone.aqxj.utils.DateUtils;
 ...
 */
public class QuyangFragment extends BaseLazyFragment {

    private static final String TAG = QuyangFragment.class.getSimpleName();
    ...
    private List<QuyangFragmentListData.DataBean> listData;
    private QuyangFragmentListData itemsData;
    private QuyangFragmentRVAdapter mAdapter;
    private DepartmentData mDepartmentData;
    public static QuyangFragment newInstance() {
        return new QuyangFragment();
    }
...
//        initToolbarMenu(mToolbar);
        ...
        //设置动画与适配器
        SlideInLeftAnimationAdapter mSlideInLeftAnimationAdapter = new SlideInLeftAnimationAdapter(mAdapter = new QuyangFragmentRVAdapter(_mActivity, listData));
    ...
    public String createRefreshULR() {
        ...
        if (null != mParametersData) {
            ...
            startDateTime = DateUtils.ChangeTimeToLong(mParametersData.startDateTime);
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.CHINA);
            Calendar rld = Calendar.getInstance();
            endDateTime = DateUtils.ChangeTimeToLong(sdf.format(rld.getTime()));
...
//        return URL.getPeiliaotongzhidan(userGroupID, startDateTime, endDateTime, currentPage,maxPageItems );
        return URL.getTaizhangListData( startDateTime, endDateTime, currentPage,maxPageItems );
    }
...
//        return URL.getPeiliaotongzhidan(userGroupID, startDateTime, endDateTime, currentPage,maxPageItems );
    ...
    public void onRefreshSuccess(String response) {
        if (!TextUtils.isEmpty(response)) {
            ...
            if (jsonObject.optBoolean("success")) {
                itemsData = mGson.fromJson(response, QuyangFragmentListData.class);
                if (null != itemsData) {
    ...
    private void jump2DetailActivity(int position) {
        Intent intent = new Intent(_mActivity, YangbenListActivity.class);
        Bundle bundle = new Bundle();
        bundle.putSerializable("quyangDetail", listData.get(position));
        intent.putExtras(bundle);
        startActivity(intent);
    }
