...
import com.shtoone.aqxj.utils.ScreenUtils;
...
import static com.shtoone.aqxj.BaseApplication.context;
                    ...
                    if (itemsData.getSuccess() && itemsData.getData().size() > 0) {
                        ...
                        if (null != listData) {
                            if (listData.size() > 0) {
                                ...
                                if (mUserInfoData.getQuanxian().isWZTJ()) {
                                    ...
                                    Toast.makeText(getContext(),"请在平台配置通知单审核权限",Toast.LENGTH_LONG).show();
                                }
                                mRecyclerView.setAdapter(mScaleInAnimationAdapter);
                            } else {
                                //提示数据为空，展示空状态
